...
{
    private const string ErrormessPropertyDiffSet = "Properties differs between the table and the set";
    private const string ErrormessInstancetableFormat = "Can only create instances of tables with one row, or exactly 2 columns and several rows";
    private const string ErrormessNotOnTable = "The '{0}' value not present in the table, but on the instance";
    private const string ErrormessNotOnInstance = "The '{0}' value not present on the instance, but in the table";
    private const string ErrormessValueDiffers =
        "The '{0}' value differs from table and instance.\n\tInstance:\t'{1}'(type: {2}).\n\tTable:\t\t'{3}'(type: {4})";

    private const string ErrormessNumberOfRowsDiffers =
        "Number of rows for table ({0} rows) and set ({1} rows) differs";

    private const string ErrormessSetValuesDiffers =
    ...
    public static ExpandoObject CreateDynamicInstance(this Table table, bool doTypeConversion = true)
    {
        ...
        throw new DynamicInstanceFromTableException(ErrormessInstancetableFormat);
    }
    ...
    public static void CompareToDynamicSet(this Table table, IList<dynamic> set, bool doTypeConversion = true)
    {
        ...
        if (propDiffs.Any())
        {
            throw new DynamicSetComparisonException(ErrormessPropertyDiffSet, propDiffs);
        }
        ...
        {
            throw new DynamicSetComparisonException(ErrormessPropertyDiffSet, valueDifference);
        }
    }
    ...
    private static List<string> GetSetValueDifferences(Table table, IList<object> set, bool doTypeConversion = true)
    {
        ...
        for (var i = 0; i < set.Count; i++)
        {
            foreach (var memberName in memberNames)
            {
                ...
                var instanceValue = InvokeGet(set[i], memberName);
                var instanceType = instanceValue.GetType().Name;

                if (!instanceValue.Equals(rowValue))
                {
                    var difference = string.Format(ErrormessSetValuesDiffers,
                        i + 1,
                        currentHeader,
                        memberName,
                        instanceValue,
    ...
    private static void AssertEqualNumberOfRows(Table table, IList<object> set)
    {
        ...
        {
            var mess = string.Format(ErrormessNumberOfRowsDiffers, table.RowCount, set.Count);
    ...
    private static IList<string> ValidateValuesOfRow(DataTableRow tableRow, dynamic instance, bool doTypeConversion = true)
    {
        ...
        foreach (var header in tableRow.Keys)
        {
            ...
            var valueFromInstance = InvokeGet(instance, propertyName);
            ...
            {
                var mess = string.Format(ErrormessValueDiffers, propertyName, valueFromInstance, typeFromInstance, valueFromTable, typeFromTable);
                valueDiffs.Add(mess);
            }
        }
        return valueDiffs;
    ...
    private static IList<string> GetPropertyNameDifferences(IEnumerable<string> tableHeadersAsPropertyNames, IEnumerable<string> instanceMembers)
    {
        ...
        diffs.AddRange(
            allMembersInInstanceButNotInTable.Select(
                m => string.Format(ErrormessNotOnTable, m)));

        diffs.AddRange(
            allMembersInTableButNotInInstance.Select(
                m => string.Format(ErrormessNotOnInstance, m)));
